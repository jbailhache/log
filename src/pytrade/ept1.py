...
def epf(intraday,tr,t1,periode,n,tau0,tau1):
  master=Tk()
  ...
  for t in range(imin,imax):
    ...
    for o in range (n):
      ...
      d = d + 1.0/(tau*tau) * (phi[o] * math.cos((t-tr)/tau) - psi[o] * math.sin((t-tr)/tau))
  ...
  for t in range(imin,imax):
    ...
    for o in range (n):
      tau = tau0 + (tau1 - tau) * o / (n - 1)
      # d = d + phi[o] * math.cos (t / tau) + psi[o] * math.sin (t / tau) 
      d = d + 1.0/(tau*tau) * (phi[o] * math.cos((t-tr)/tau) - psi[o] * math.sin((t-tr)t/tau))
      # d = d + 1.0/tau * (phi[o] * math.cos(t/tau) + psi[o] * math.sin(t/tau))
    # d = d / n 
    d = 1/(2*3.1415926) * (tau1-tau0)/(n-1) * d
