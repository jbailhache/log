...
Differential game of pursuit

A pursuer tries to go as near as possible to an escaper who tries to keep as far as possible from the escaper. The pursuer is limited in direction changing which has a cost. The escaper can move in any direction without cost. Both pursuer and escaper move at constant speeds. 

The state is described by the variables :
x1, y1 : position of the pursuer
x2, y2 : position of the escaper
a : angle of moving of the pursuer

We can simplify the problem by introducing variables x and y defined by : 
x = x2 - x1
y = y2 - y1

The control of the pursuer is u1, the variation of direction of the pursuer.
The control of the escaper is u2, the direction of the escaper.

The evolution of the state is described by the equations : 
dx/dt = -v1 cos a + v2 cos u2
dy/dt = -v1 sin a + v2 sin u2
da/dt = v1/L u1 

The cost which the pursuer wants to minimize is : u1^2 + x^2 + y^2
The cost which the escaper wants to minimize is : -x^2 - y^2

The hamiltonian of the pursuer is : 
H1 = u1^2 + x^2 + y^2 + px1 (-v1 cos a + v2 cos u2) + py2 (-v1 sin a + v2 sin u2) + pa1 v1/L u1

The hamiltonian of the escaper is : 
H2 = -x^2 - y^2 + px2 (-v1 cos a + v2 cos u2) + py2 (-v1 sin a + v2 sin u2) + pa2 v1/L u1

The optimality conditions according to HJBI equation and Pontryagin principle are : 
dpx1/dt = -dH1/dx = -2x
dpy1/dt = -dH1/dy = -2y
dpa1/dt = -dH1/da = v1 (-px1 sin a + py1 cos a)
dpx2/dt = -dH2/dx = 2x
dpy2/dt = -dH2/da = v1 (-px2 sin a + py2 cos a)
dH1/du = 0 = 2 u1 + pa1 v1/L => u1 = -1/2 pa1 v1/L 
dH2/du2 = 0 = v2 (-px2 sin u2 + py2 cos u2) => u2 = arg (px2 + i py2) 
...
=end
  ...
  0.upto(nt) do |i|
   ...
   px1 = px1 + @ft * dt * (-2*x)
   py1 = py1 + @ft * dt * (-2*y)
   pa1 = pa1 + @ft * dt * (v1*(-px1*Math.sin(a)+py1*Math.cos(a)))

   px2 = px2 + @ft * dt * (2*x)
   py2 = py2 + @ft * dt * (2*y)
   pa2 = pa2 + @ft * dt * (v1*(-px2*Math.sin(a)+py2*Math.cos(a)))
...
=end 
  ...
  text = "%7.4f" % @dp
  canvas.drawText(text,40,390,@paint4)

  text = "%6.2f ; %6.2f" % [@slo1, sl1] 
  canvas.drawText(text,40,420,@paint4)

  text = "%6.2f ; %6.2f" % [@slo2, sl2] 
  canvas.drawText(text,40,450,@paint4)

  text = "%6.3f ; %6.3f" % [@h1, @h2]
  canvas.drawText(text,40,480,@paint4)

  text = "%6.3f ; %6.3f ; %6.3f" % @p1 
  canvas.drawText(text,40,510,@paint4) 
 
  text = "%6.3f ; %6.3f ; %6.3f" % @po1 
  canvas.drawText(text,40,540,@paint4) 
 
  text = "%6.3f ; %6.3f ; %6.3f" % @p2
  canvas.drawText(text,40,570,@paint4) 
 
  text = "%6.3f ; %6.3f ; %6.3f" % @po2
  canvas.drawText(text,40,600,@paint4) 
