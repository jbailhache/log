...
def depl_afch(d):
  ...
  for h in histsj:
    ...
    vib = coef * (h[3] - ex)
    vic = coef * (h[4] - ex)
    # print 'c=',h[4],' ex=',ex,' coef=',coef,' vio=',vio,' vih=',vih,' vib=',vib,' vic=',vic
    yo = hg - hg * (vio-chmin) / (chmax-chmin)
    yh = hg - hg * (vih-chmin) / (chmax-chmin)
    yb = hg - hg * (vib-chmin) / (chmax-chmin)
    yc = hg - hg * (vic-chmin) / (chmax-chmin)
    yh = int(max(0,min(hg-1,yh)))
    yb = int(max(0,min(hg-1,yb)))
    # print 'yh=',yh,' yb=',yb
    # w.create_rectangle (x-1, yh, x+1, yb, fill='purple')
    col='purple'
    ...
    #   premier = 0
    # else:
    #   w.create_line (xp, yp, x, y, fill='purple')
