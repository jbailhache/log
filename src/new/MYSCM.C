...
pointer s_return2 (a) pointer a;
{
 s_return1(a);
}

#define s_return(a) { return s_return2(a); }
...
#define OP_STRINGEQ     107
#define OP_STRINGREF    108
#define OP_LISTSTRING   109
...
pointer bind (pointer vars, pointer args, pointer env)
{
	if (isfalse(vars))
		return env;
	if (!ispair(vars))
	...
	if (ispair(args))
		return bind (car(vars), car(args),
			bind (cdr(vars), cdr(args), env));
	return bind (car(vars), args,
		bind (cdr(vars), args, env));
}
...
pointer opexe_7 (op) register short op;
{
	...
	case OP_DOMLAMBDA:
		...
		*/
		...
		break;
        case OP_STRINGEQ:
		/* s_retbool(!strcmp(strvalue(car(args)),strvalue(car(cdr(args))))); */
...
register short op;
{
	...
	{
		/*
		if (operator != OP_LOAD && names[operator])
			printf (" %s ", names[operator]);
		*/
