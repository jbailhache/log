...
main_search (int argc, char *argv[])
{
...
main ()
{
	favoris ("c:\\windows\\favoris", "*.*");
}

favoris (char *dir, char *name)
{
struct ffblk blk;
int attrib, status;
char attributs[10];
...
int i;
	...
	while (status >= 0)
	{
		...
		else
		{
			...
			{
				strcpy (filename, blk.ff_name);
				filename [strlen(filename)-4] = 0;
				printf ("<li>%s : <a href=\"%s\">%s</a>\n",
					filename, line+4, line+4);
				break;
			}
		    }
		    fclose (f);
