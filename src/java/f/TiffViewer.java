...
import lizard.tiff.*;
    ...
    public class TiffViewer {
	  ...
	  int StripOffsets;
	  int stripoffset = 274;
	  char type; // S = standard, P = propriétaire
	  ...
	  public int get_byte_t4 ()
	  {
		...
		return rev(unsign(data[ptr++]));
                ...
                {
                        // System.out.println("Cannot read fax byte:" + e.toString());
                        return EOT4;
                }*/
        }
	  ...
	  public int get_code_t4_a (int n)
		...
		while ((pos_code_t4 > (32 - maxbits)) && !eot4()) 	/* tant qu'on ne dépasse pas maxbits et qu'il y a encore des bits */
		{
			pos_code_t4 -= 8;						/* on avance le pointeur pour lire 1 octet */
                        ...
                        //if (b == EOT4)
                        //        return EOT4;
                        code_t4 |= b << pos_code_t4;                /* on place l'octet lu dans code_t4 */
		}
	  ...
	  */
	  ...
	  public void convert () 
	  {
		...
		try
		{
					...
					values[256] = 1728; imageWidth = 1728;
					values[257] = 1151; imageHeight = 1151;
					values[258] = 1;
					values[259] = 3;
					values[262] = 0;
					values[266] = 2;
					values[273] = 266; StripOffsets = 266;
					values[277] = 1;
					values[278] = 0;
					values[279] = 0;
					values[282] = 250;
					...
					for (int i=0; i<dec; i++)
					{
						...
						System.out.println ("DE " + i + ": " + tag + " " + typ + " " + lng + " " + val);
					...
					if (imageHeight < 1218) imageHeight = 1218;
					// if (imageHeight < 2400) imageHeight = 2400;

					/*
					if (imageHeight > 1300)
						imageHeight = 1300;
					*/
					if (imageHeight > 2300) imageHeight = 2300; 
		...
		{
			e.printStackTrace();
			System.out.println ("Error convert: " + e.toString());
		}
        }
	  ...
	  /** Dessine l'image */
 	  public void drawImageCreated (Graphics g, int x, int y, int width, int height)
	  {
		/*createImage (background, ink);*/
		Image img1 = img.getScaledInstance (width, height, img.SCALE_SMOOTH); 
		g.drawImage (img1, x, y, null);
