		...
		try
		{
			...
			for(;;)
			{
				...
				else if (commande.equals("FAXRCV"))
				{
					...
					catch (Exception e) { I = new Integer (-1); }
					int rtr = I.intValue();
					...
					String called = recoit_param (is, "any");
					String calling = recoit_param (is, "any");
					...
					String s_hng = recoit_param (is, "any");
					try { I = new Integer (s_hng); }
					catch (Exception e) { I = new Integer (-1); }
					int hng = I.intValue();

					String s_duration = recoit_param (is, "any");
					try { I = new Integer (s_duration); }
					catch (Exception e) { I = new Integer (-1); }
					int duration = I.intValue();

					String s_pagenb = recoit_param (is, "any");
					try { I = new Integer (s_pagenb); }
					catch (Exception e) { I = new Integer (-1); }
					int pagenb = I.intValue();

					String rfn = recoit_param (is, "any");
					String xdate = recoit_param (is, "any");
					String xtime = recoit_param (is, "any");
					...
					{	
						...
						while (rs.next())
						{
 							    ...
 							    (calling1 == null || calling1.equals("") || calling1.equals(calling))) 
							{
								...
								String src = "{ (FAXINFO #" + rtr + "# #" + line + "# \"" + id
								+ "\" \"" + called + "\" \"" + calling + "\" #"
								+ status + "# #" + hng + "# #" + duration + "# #"
								+ pagenb + "# \"" + rfn + "\" \"" + xdate 
								+ "\" \"" + xtime + "\") " + code1.replace('$',' ') + "  bye HALT }";
