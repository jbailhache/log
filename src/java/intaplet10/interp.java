...
class interp
{
  ...
  */
  ...
  else if (op.equals ("mkexprse"))
  {
   String n = pile.niveau(0).noeud;
   int a = (int)(pile.niveau(1).valeur);
   expr x = new expr (n, a);
   for (int i=0; i<a; i++)
    x.sousexpr[i] = pile.niveau(2+i);
   r = pushctx (x, popctx (2+a, ctx));
  }
  ...
  */
  ...
  else if (op.equals ("getargs"))
  {
	r = pushctx (args, ctx);
  }
  else if (op.equals ("setargs"))
  {
	r = new expr ("CTX", cont, pile.sousexpr[1], pile.sousexpr[0], env);
  }
  else if (op.equals ("evalwith"))
  {
	r = new expr ("CTX", new expr ("SEQ", pile.niveau(0), new expr ("SEQ",
		new expr ("SEQ", new expr ("qt", args), new expr("setargs")),
		cont)), pile.sousexpr[1].sousexpr[1], pile.niveau(1), env);
  }
  else if (op.equals ("close"))
  {
	r = new expr ("CTX", cont, 
		new expr ("PUSH", new expr ("closure", args, instr.sousexpr[0]), pile),
		args, env);
  }
  else if (op.equals ("closure"))
  {
	r = new expr ("CTX", new expr ("SEQ", new expr ("SEQ", instr.sousexpr[1], 
		new expr ("SEQ", new expr ("qt", args), new expr("setargs"))), cont),
		pile, instr.sousexpr[0], env);
  }
  /* idee : 
     dans le cas ou instr.sousexpr[1] rend 1 valeur, 
     ajouter 3eme sousexpr = (no) ou (yes x)
     si (yes x) rendre x
     si (no) idem et remplacer 3e sousexpr par (yes pile.niveau(0))
  */
 else if (op.equals ("memo"))
 {
	r = new expr ("CTX", new expr ("SEQ", new expr ("SEQ", instr.sousexpr[0],
		new expr ("memorize", instr)), cont),
		pile, args, env);
 }
 else if (op.equals ("memorize"))
 {
	instr.sousexpr[0].noeud = "qt";
	instr.sousexpr[0].sousexpr[0] = pile.niveau(0);
	r = new expr ("CTX", cont, pile, args, env);
 }
...
*/
  ...
  else if (op.equals ("mkfax"))
  {
	...
	String filename = pile.niveau(8).chaine;
	expr pages = pile.niveau(9);

	System.out.println ("basefax=" + basefax + " baseresend=" + baseresend 
		+ " faxnum=" + numfax + " retry=" + retry + " pri=" + pri 
		+ " hash=" + hash + " delay=" + delay + " flid=" + flid + " filename=" + filename);
	...
	r = pushctx (new expr(fax), popctx(10,ctx));
    }
    catch (Exception e)
    {
		System.out.println ("Error mkfax:" + e.toString());
		r = pushctx (new expr("STR",-1,e.toString()), popctx(10,ctx));
    }
  }
  else if (op.equals ("sendfax"))
  {
	...
	Fax fax = (Fax)(pile.niveau(4).objet);

	System.out.println ("host=" + host + " port=" + port + " login=" + login);

	/*fax.send();*/
	/*fax.send(false);*/
	ServerConnection sc = new ServerConnection (host, port);
	System.out.println ("login");
	sc.login (fax.basefax, login, password);
