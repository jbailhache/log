...
struct s_cours { char *nom; float cours[500/*120*/]; };

struct s_cours tab_cours1[] = {
/* Castorama */
/* float cours[] = */
...
#define NVALEURS (sizeof(tab_cours1)/sizeof(tab_cours1[0]))

struct s_cours tab_cours [NVALEURS];
...
int i, j, k, l;
	l = 0;
	for (i=0; i<NVALEURS; i++)
	{
		tab_cours[i].nom = tab_cours1[i].nom;
		for (j=0; tab_cours1[i].cours[j-1]; j++)
		{
			for (k=0; k<ntr; k++)
			{
				tab_cours[i].cours[l=j*3+k] =
					(tab_cours1[i].cours[j] * (ntr-k) +
					 tab_cours1[i].cours[j+1] * k) / ntr;
			}
		}
		tab_cours[i].cours[l+1] = 0;
	}
}
