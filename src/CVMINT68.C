...
#define code_getword 0x1200FF
#define code_setbradr 0x130000
...
void setbradr (instr *from, instr *to)
{
 int ofs;
 ofs = to - (from + 3);
 printf("from=%X to=%X ofs=%X *from=%X\n",
  from,to,ofs,*from);
 /* *from |= ofs; */
...
codeptr c_mov_r2_r0 (codeptr cp)
{
 if (f_tasm != NULL)
  fprintf (f_tasm," mov cx,ax\n");
 if (f_asmi != NULL)
  fprintf (f_asmi, " mov %%cx,%%ax\n");
 if (f_asml != NULL)
  fprintf (f_asml, " mov %%ax,%%cx\n");
 if (f_x86 != NULL)
  fprintf (f_x86, " .byte 0x89, 0xC1\n");
#ifdef INTERP
 *cp++ = code_mov_r2_r0;
#endif
#ifdef COMPARM
...
codeptr c_mov_r0_r2 (codeptr cp)
{
 if (f_tasm != NULL)
  fprintf (f_tasm," mov ax,cx\n");
 if (f_asmi != NULL)
  fprintf (f_asmi, " mov %%ax,%%cx\n");
 if (f_asml != NULL)
  fprintf (f_asml, " mov %%cx,%%ax\n");
 if (f_x86 != NULL)
  fprintf (f_x86, " .byte 0x89, 0xC8\n");
#ifdef INTERP
 *cp++ = code_mov_r0_r2;
#endif
#ifdef COMPARM
...
codeptr c_mov_r1_r2 (codeptr cp)
{
 if (f_tasm != NULL)
  fprintf (f_tasm," mov dx,cx\n");
 if (f_asmi != NULL)
  fprintf (f_asmi, " mov %%dx,%%cx\n");
 if (f_asml != NULL)
  fprintf (f_asml, " mov %%cx,%%dx\n");
 if (f_x86 != NULL)
...
codeptr c_mov_ra_r0 (codeptr cp)
{
 if (f_tasm != NULL)
...
codeptr c_mov_rb_r0 (codeptr cp)
{
 if (f_tasm != NULL)
  fprintf (f_tasm," mov bx,ax\n");
 if (f_asmi != NULL)
  fprintf (f_asmi, " mov %%bx,%%ax\n");
 if (f_asml != NULL)
  fprintf (f_asml, " mov %%ax,%%bx\n");
 if (f_x86 != NULL)
  fprintf (f_x86, " .byte 0x89, 0xC3\n");
#ifdef INTERP
 *cp++ = code_mov_rb_r0;
#endif
#ifdef COMPARM
...
codeptr c_mov_rc_r0 (codeptr cp)
{
 if (f_tasm != NULL)
  fprintf (f_tasm," mov di,ax\n");
 if (f_asmi != NULL)
  fprintf (f_asmi, " mov %%di,%%ax\n");
 if (f_asml != NULL)
  fprintf (f_asml, " mov %%ax,%%di\n");
 if (f_x86 != NULL)
  fprintf (f_x86, " .byte 0x89, 0xC7\n");
#ifdef INTERP
 *cp++ = code_mov_rc_r0;
#endif
#ifdef COMPARM
...
codeptr c_mov_r0_ra (codeptr cp)
{
 if (f_tasm != NULL)
...
codeptr c_mov_r0_rb (codeptr cp)
{
 if (f_tasm != NULL)
  fprintf (f_tasm," mov ax,bx\n");
 if (f_asmi != NULL)
  fprintf (f_asmi, " mov %%ax,%%bx\n");
 if (f_asml != NULL)
  fprintf (f_asml, " mov %%bx,%%ax\n");
 if (f_x86 != NULL)
  fprintf (f_x86, " .byte 0x89, 0xD8\n");
#ifdef INTERP
 *cp++ = code_mov_r0_rb;
#endif
#ifdef COMPARM
...
codeptr c_mov_r0_rc (codeptr cp)
{
 if (f_tasm != NULL)
  fprintf (f_tasm," mov ax,di\n");
 if (f_asmi != NULL)
  fprintf (f_asmi, " mov %%ax,%%di\n");
 if (f_asml != NULL)
  fprintf (f_asml, " mov %%di,%%ax\n");
 if (f_x86 != NULL)
  fprintf (f_x86, " .byte 0x89, 0xF8\n");
#ifdef INTERP
 *cp++ = code_mov_r0_rc;
#endif
#ifdef COMPARM
...
codeptr c_mov_ra_r1 (codeptr cp)
{
 if (f_tasm != NULL)
  fprintf (f_tasm," mov si,dx\n");
 if (f_asmi != NULL)
  fprintf (f_asmi, " mov %%si,%%dx\n");
 if (f_asml != NULL)
  fprintf (f_asml, " mov %%dx,%%si\n");
 if (f_x86 != NULL)
  fprintf (f_x86, " .byte 0x89, 0xD6\n");
#ifdef INTERP
 *cp++ = code_mov_ra_r1;
#endif
#ifdef COMPARM
...
codeptr c_mov_rc_r1 (codeptr cp)
{
 if (f_tasm != NULL)
  fprintf (f_tasm," mov di,dx\n");
 if (f_asmi != NULL)
  fprintf (f_asmi, " mov %%di,%%dx\n");
 if (f_asml != NULL)
  fprintf (f_asml, " mov %%dx,%%di\n");
 if (f_x86 != NULL)
  fprintf (f_x86, " .byte 0x89, 0xD7\n");
#ifdef INTERP
 *cp++ = code_mov_rc_r1;
#endif
#ifdef COMPARM
...
codeptr c_mov_rc_ra (codeptr cp)
{
 if (f_tasm != NULL)
  fprintf (f_tasm," mov di,ax\n");
 if (f_asmi != NULL)
...
codeptr c_mov_r1_rc (codeptr cp)
{
 if (f_tasm != NULL)
  fprintf (f_tasm," mov dx,di\n");
 if (f_asmi != NULL)
  fprintf (f_asmi, " mov %%dx,%%di\n");
 if (f_asml != NULL)
  fprintf (f_asml, " mov %%di,%%dx\n");
 if (f_x86 != NULL)
  fprintf (f_x86, " .byte 0x89, 0xFA\n");
#ifdef INTERP
 *cp++ = code_mov_rc_r1;
#endif
#ifdef COMPARM
...
{
 if (f_tasm != NULL)
  fprintf (f_tasm, " call %c\n", last_label);
 if (f_asmi != NULL)
  fprintf (f_asmi, " call %c\n", last_label);
 if (f_asml != NULL)
  fprintf (f_asml, " call %c\n", last_label);
 if (f_x86 != NULL)
  fprintf (f_x86, " call %c\n", last_label);
#ifdef INTERP
 *cp++ = code_call_depl;
 *cp++ = depl;
#endif
...
any a;
 ...
 else if (!strcmp (op, "r=r"))
 {
  if (iargs[0]==1 && iargs[1]==0)
   cp = c_mov_r1_r0 (cp);
  else if (iargs[0]==2 && iargs[1]==0)
   cp = c_mov_r2_r0 (cp);
  else if (iargs[0]==0 && iargs[1]==2)
   cp = c_mov_r0_r2 (cp);
  else if (iargs[0]==1 && iargs[1]==2)
   cp = c_mov_r1_r2 (cp);
  else if (iargs[0]==10 && iargs[1]==0)
   cp = c_mov_ra_r0 (cp);
  else if (iargs[0]==11 && iargs[1]==0)
   cp = c_mov_rb_r0 (cp);
  else if (iargs[0]==12 && iargs[1]==0)
   cp = c_mov_rc_r0 (cp);
  else if (iargs[0]==0 && iargs[1]==10)
   cp = c_mov_r0_ra (cp);
  else if (iargs[0]==0 && iargs[1]==11)
   cp = c_mov_r0_rb (cp);
  else if (iargs[0]==0 && iargs[1]==12)
   cp = c_mov_r0_rc (cp);
  else if (iargs[0]==10 && iargs[1]==1)
   cp = c_mov_ra_r1 (cp);
  else if (iargs[0]==12 && iargs[1]==1)
   cp = c_mov_rc_r1 (cp);
  else if (iargs[0]==12 && iargs[1]==10)
   cp = c_mov_rc_ra (cp);
  else if (iargs[0]==1 && iargs[1]==12)
   cp = c_mov_r1_rc (cp);
  else
   printf ("Invalid instruction <%s>\n", s);
 }
